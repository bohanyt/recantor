...
function Assert-Equal { param($Actual,$Expected,[string]$Message); if($Actual -ne $Expected){throw "ASSERT EQUAL FAILED: $Message. Expected '$Expected', got '$Actual'."} }
function Assert-Throws {
    param([scriptblock]$Action,[string]$Pattern,[string]$Message)
    $threw=$false
    try{& $Action}catch{$threw=$true;if($Pattern -and $_.Exception.Message -notmatch $Pattern){throw "ASSERT THROWS FAILED: $Message. Wrong message: $($_.Exception.Message)"}}
    if(-not $threw){throw "ASSERT THROWS FAILED: $Message"}
}
function New-TestManifest {
    param([int]$Format=2,[string]$Version="v0.1.0-alpha.3",[string]$Channel="alpha",[string[]]$SafeTo=@("v0.1.0-alpha.2"),[bool]$BackupRequired=$false)
    $compat=[pscustomobject]@{platforms=@("linux/amd64");schema_head="0009";automatic_database_downgrade=$false}
    if($Format -eq 2){Add-Member -InputObject $compat -NotePropertyName application_rollback_safe_to_versions -NotePropertyValue $SafeTo;Add-Member -InputObject $compat -NotePropertyName backup_required -NotePropertyValue $BackupRequired}
    return[pscustomobject]@{
        format_version=$Format
        version=$Version
        channel=$Channel
        source=[pscustomobject]@{repository="https://github.com/bohanyt/recantor";revision=("a"*40)}
        images=[pscustomobject]@{
            api=[pscustomobject]@{repository="ghcr.io/bohanyt/recantor-api";digest=("sha256:"+("b"*64));reference=("ghcr.io/bohanyt/recantor-api@sha256:"+("b"*64))}
            web=[pscustomobject]@{repository="ghcr.io/bohanyt/recantor-web";digest=("sha256:"+("c"*64));reference=("ghcr.io/bohanyt/recantor-web@sha256:"+("c"*64))}
        }
