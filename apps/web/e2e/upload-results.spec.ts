...
test('reload during processing resumes the same capability and reaches visible canonical transcript', async ({
  page,
}) => {
  ...
  await page.route(`**/api/v1/uploads/${sessionId}/result`, async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(resultBody(state)) });
  });
...
test('410 clears browser capability recovery, stops polling, and tells the truth about server data', async ({
  page,
}) => {
  await seedRecovery(page);
  ...
  await expect(page.getByTestId('upload-message')).toContainText('server recording was not deleted');
  expect(await page.evaluate((key) => window.localStorage.getItem(key), storeKey)).toBe('{}');
  await page.waitForTimeout(1_300);
  expect(resultRequests).toBe(1);
});

test('temporary result fetch failure reconnects without changing the saved session', async ({ page }) => {
  await seedRecovery(page);
  let attempts = 0;
  await page.route(`**/api/v1/uploads/${sessionId}/result`, async (route) => {
    attempts += 1;
  ...
  await expect(page.getByTestId('upload-phase')).toHaveText('No speech detected', { timeout: 5_000 });
